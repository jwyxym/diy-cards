...
function s.cfilter(c)
	return (c:IsSetCard(0x763) or c:IsRace(RACE_PLANT)) and c:IsFaceup()
end
...
--特殊召唤值：返回1表示守备表示，zone表示可召唤区域
function s.hspval(e,c)
	local tp=c:GetControler()
	return 1,s.getzone(tp)
end
function s.desfilter(c)
	return c:IsSetCard(0x763) or c:IsRace(RACE_PLANT)
end
function s.spfilter(c,e,tp)
	return c:IsSetCard(0x763) and not c:IsCode(id) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then
		...
		e:SetLabel(loc)
		return Duel.IsExistingTarget(s.desfilter,tp,loc,0,1,nil)
			and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,s.desfilter,tp,e:GetLabel(),0,1,1,nil)
...
function s.splimit(e,c)
	return not (c:IsRace(RACE_PLANT) and c:IsType(TYPE_PENDULUM)) and not c:IsLocation(LOCATION_EXTRA)
end
