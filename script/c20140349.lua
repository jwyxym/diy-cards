...
function s.acttg(e, tp, eg, ep, ev, re, r, rp, chk)
    if chk==0 then return e:IsCostChecked()
		...
		and Duel.IsPlayerCanSpecialSummonMonster(tp, id, 0x1c6, TYPES_NORMAL_TRAP_MONSTER, 1700, 1300, 4, RACE_FIEND, ATTRIBUTE_DARK, POS_FACEUP, tp)
    end
...
function s.actop(e, tp, eg, ep, ev, re, r, rp)
    local c = e:GetHandler()
    if not Duel.IsPlayerCanSpecialSummonMonster(tp, id, 0x1c6, TYPES_NORMAL_TRAP_MONSTER, 1700, 1300, 4, RACE_FIEND, ATTRIBUTE_DARK, POS_FACEUP, tp) then 
        return 
    end
    ...
    if Duel.SpecialSummon(c, 0, tp, tp, true, false, POS_FACEUP) ~= 0 then
        if s.gravecheck(e, tp) and Duel.IsExistingMatchingCard(Card.IsAbleToRemove, 1 - tp, LOCATION_MZONE, 0, 1, nil)
            and Duel.SelectYesNo(tp, aux.Stringid(id, 0)) then
            Duel.BreakEffect()
            Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_REMOVE)
            local g = Duel.SelectMatchingCard(tp, Card.IsAbleToRemove, 1 - tp, LOCATION_MZONE, 0, 1, 1, nil)
            if #g > 0 then
...
function s.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
    ...
    if chk == 0 then
        return Duel.IsExistingTarget(s.tgfilter, tp, LOCATION_REMOVED, 0, 1, nil) end
...
function s.tgfilter(c, e,tp)
    return c:IsSetCard(0x1c6) and not c:IsCode(id) and c:IsFaceup()
end