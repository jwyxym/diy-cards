...
function s.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_TODECK+CATEGORY_SPECIAL_SUMMON+CATEGORY_FUSION_SUMMON+CATEGORY_DISABLE)
...
function s.filter2(c,e,tp,m,f,chkf)
	return c:IsType(TYPE_FUSION) and c:IsRace(RACE_WARRIOR) and c:IsAttribute(ATTRIBUTE_DARK+ATTRIBUTE_FIRE) and (not f or f(c))
...
function s.spfilter(c,e,tp)
	return c:IsFaceup() and c:IsType(TYPE_FUSION) and c:IsRace(RACE_WARRIOR) and c:IsAttribute(ATTRIBUTE_DARK+ATTRIBUTE_FIRE) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
