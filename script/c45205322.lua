...
function s.initial_effect(c)
    ...
    c:EnableReviveLimit()
    -- ★★★ 修正：连接素材必须是4星以上的侦探怪兽 ★★★
    aux.AddLinkProcedure(c, s.matfilter, 1, 1, aux.CheckOpMaximum, 4, 99)
    
    --①效果：连接召唤成功时，从卡组特殊召唤1只「侦探」怪兽
...
function s.matfilter(c, lc, tp, sg)
    return c:IsSetCard(0x1D5C) and c:IsType(TYPE_MONSTER) and c:GetLevel()>=4
end
