...
local MOVE1 = {LINK_MARKER_BOTTOM, LINK_MARKER_BOTTOM_LEFT, LINK_MARKER_LEFT}
local MOVE2 = {LINK_MARKER_RIGHT, LINK_MARKER_BOTTOM_RIGHT, LINK_MARKER_BOTTOM, LINK_MARKER_BOTTOM_LEFT, LINK_MARKER_LEFT}

function s.initial_effect(c)
	aux.AddCodeList(c,45200113) -- 格斗天王 高尼兹
...
function s.tg2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		local b1 = Duel.IsExistingMatchingCard(s.windfilter,tp,LOCATION_HAND+LOCATION_DECK,0,1,nil)
		local b2 = Duel.IsExistingMatchingCard(nil,tp,0,LOCATION_MZONE,1,nil)
			and Duel.IsExistingMatchingCard(s.windfilter,tp,LOCATION_HAND+LOCATION_DECK,0,1,nil)
		return b1 or b2
	end
end
...
function s.op2(e,tp,eg,ep,ev,re,r,rp)
	...
	local seq
	if sel==1 then seq=MOVE1 else seq=MOVE2 end
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONFIRM)
	local ag=Duel.SelectMatchingCard(tp,s.windfilter,tp,LOCATION_HAND+LOCATION_DECK,0,1,1,nil)
	if #ag==0 then
		Duel.Damage(tp,100,REASON_EFFECT)
		return
	end
	...
	if sel==1 then
		...
		if #dg>0 then
			...
			if Duel.IsExistingMatchingCard(s.windfilter,tp,LOCATION_HAND,0,1,nil)
				and Duel.SelectYesNo(tp,aux.Stringid(id,4)) then
				Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONFIRM)
				local hg=Duel.SelectMatchingCard(tp,s.windfilter,tp,LOCATION_HAND,0,1,1,nil)
				if #hg>0 then
