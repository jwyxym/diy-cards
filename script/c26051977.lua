...
-- 链接条件：场上至少2只怪兽，至少1只「布若」
function s.linkcon(e,c,og,lmat,min,max)
    if c==nil then return true end
    local tp=c:GetControler()
    local g = Duel.GetMatchingGroup(Card.IsFaceup, tp, LOCATION_MZONE, 0, nil)
    if #g < 2 then return false end
    return g:FilterCount(Card.IsSetCard, nil, 0x908) >= 1
end
-- 两步选择：第1步必选「布若」，第2步选其他
function s.linkop(e,tp,eg,ep,ev,re,r,rp,c,og,lmat,min,max)
    if c==nil then return end
    local g = Duel.GetMatchingGroup(Card.IsFaceup, tp, LOCATION_MZONE, 0, nil)
    local honka = g:Filter(Card.IsSetCard, nil, 0x908)
...
function s.thfilter(c)
    return c:IsSetCard(0x906) and c:IsType(TYPE_TRAP) and c:IsAbleToHand()
end
...
function s.fieldfilter(c)
    return c:IsFaceup() and (c:IsSetCard(0x907) or c:IsSetCard(0x909))
end
