--祭神科尔努诺斯
function c72600216.initial_effect(c) 
	...
	e1:SetCountLimit(1,72600216) 
	e1:SetCondition(c72600216.tkcon)
	e1:SetTarget(c72600216.tktg)
	e1:SetOperation(c72600216.tkop) 
	c:RegisterEffect(e1) 
	--des 
	...
	e2:SetCountLimit(1)
	e2:SetCondition(c72600216.descon) 
	e2:SetCost(c72600216.descost)
	e2:SetTarget(c72600216.destg)
	e2:SetOperation(c72600216.desop)
	c:RegisterEffect(e2) 
	--to hand 
	...
	e3:SetCountLimit(1,12600216)
	e3:SetTarget(c72600216.thtg)
	e3:SetOperation(c72600216.thop)
	c:RegisterEffect(e3)
end  
...
function c72600216.tkop(e,tp,eg,ep,ev,re,r,rp) 
...
function c72600216.descon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local bc=c:GetBattleTarget()
	return bc and bc:IsSummonType(SUMMON_TYPE_SPECIAL)  
end
function c72600216.descost(e,tp,eg,ep,ev,re,r,rp,chk) 
...
function c72600216.destg(e,tp,eg,ep,ev,re,r,rp,chk) 
	local c=e:GetHandler() 
	local bc=c:GetBattleTarget()
	if chk==0 then return bc and bc:IsSummonType(SUMMON_TYPE_SPECIAL) end  
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,bc,1,0,0) 
end
function c72600216.desop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local bc=c:GetBattleTarget()
...
function c72600216.thfilter(c)
	return (aux.IsCodeListed(c,72600200) or c:IsCode(72600200)) and c:IsAbleToHand()
end
function c72600216.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c72600216.thfilter,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_GRAVE)
end
function c72600216.thop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c72600216.thfilter,tp,LOCATION_GRAVE,0,1,1,nil)
