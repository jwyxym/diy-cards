...
function cm.operation1(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.Destroy(g,REASON_EFFECT)~=0 then
		local mg=Duel.GetMatchingGroup(aux.NecroValleyFilter(cm.rmfilter),tp,0,LOCATION_GRAVE,nil)
...
function cm.filter3(c)
	return c:IsSetCard(set) and c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsAbleToHand()
end
