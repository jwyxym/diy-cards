...
function s.rfilter(c,tp)
	return ((c:GetOriginalAttribute()&ATTRIBUTE_EARTH>0 and not c:IsLocation(LOCATION_MZONE)) or c:IsAttribute(ATTRIBUTE_EARTH)) and (c:IsControler(tp) or c:IsFaceup()) and c:IsAbleToRemoveAsCost()
end
...
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,ct,tp,LOCATION_GRAVE+LOCATION_REMOVED)
end
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	local ct=math.floor(e:GetHandler():GetAttack()/2000)
	if ct>0 and Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_GRAVE+LOCATION_REMOVED,LOCATION_GRAVE+LOCATION_REMOVED,ct,nil) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
		local g=Duel.SelectMatchingCard(tp,s.thfilter,tp,LOCATION_GRAVE+LOCATION_REMOVED,LOCATION_GRAVE+LOCATION_REMOVED,ct,ct,nil)
