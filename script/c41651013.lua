...
function s.rfilter(c,tp)
	return ((c:GetOriginalAttribute()&ATTRIBUTE_LIGHT>0 and not c:IsLocation(LOCATION_MZONE)) or c:IsAttribute(ATTRIBUTE_LIGHT)) and (c:IsControler(tp) or c:IsFaceup()) and c:IsAbleToRemoveAsCost()
end
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if (ct>1 and Duel.IsPlayerAffectedByEffect(tp,59822133))
		...
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,41651024,0,TYPES_TOKEN_MONSTER,1000,1000,1,RACE_WYRM,ATTRIBUTE_LIGHT) then return end
	for i=1,ct do
		local token=Duel.CreateToken(tp,41651024)
