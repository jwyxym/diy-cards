...
function cm.initial_effect(c)
	-- 这个卡名的①②效果1回合各能使用1次。
	-- ①：从自己场上把1只怪兽送去墓地才能发动。这张卡从手卡特殊召唤。那之后，可以选自己场上1只怪兽送去墓地。这个回合，自己不是不死族怪兽不能从额外卡组特殊召唤。
	...
	-- ②：这张卡被效果送去墓地的场合，以自己墓地1张「大墓」魔法卡为对象才能发动。那张卡盖放到自己场上。
...
function cm.cost1(e, tp, eg, ep, ev, re, r, rp, chk)
	if chk == 0 then
		return Duel.IsExistingMatchingCard(cm.filter2, tp, LOCATION_HAND + LOCATION_ONFIELD, 0, 1,
			e:GetHandler())
	end
	Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TOGRAVE)
	local g = Duel.SelectMatchingCard(tp, cm.filter2, tp, LOCATION_HAND + LOCATION_ONFIELD, 0, 1, 1,
		e:GetHandler())
...
function cm.op1(e, tp, eg, ep, ev, re, r, rp)
	...
	if Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) then
		...
		-- 	end
		-- end
		if Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(Card.IsCanBeSpecialSummoned, tp, 0, LOCATION_GRAVE, 1, nil, e, 0, tp, false, false) then
			if Duel.SelectYesNo(tp, aux.Stringid(m, 2)) then
				Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
				local g = Duel.SelectMatchingCard(tp, Card.IsCanBeSpecialSummoned, tp, 0, LOCATION_GRAVE, 1,
