...
function s.cfilter(c)
	return c:IsLocation(LOCATION_MZONE) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsSetCard(0xdf99)
...
function s.disfilter(c,lg)
	return lg:IsContains(c) and aux.NegateAnyFilter(c)
end
function s.chop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and tc:IsRelateToEffect(e) then
        ...
        local g=Duel.GetMatchingGroup(s.disfilter,tp,0,LOCATION_ONFIELD,nil,lg)
