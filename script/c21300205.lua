...
function this.regfilter(c)
	if c:IsSummonLocation(LOCATION_EXTRA) then
		return c:IsRace(RACE_CYBERSE) and c:IsType(TYPE_FUSION+TYPE_LINK)
	else return true
	end
end
function this.spfilter(c,tp,seq)
	return c:IsControler(tp) and c:GetSequence()==seq
end
function this.spcon(e,tp,eg,ep,ev,re,r,rp)
	local seq=e:GetHandler():GetSequence()
	return eg:IsExists(this.spfilter,1,nil,tp,e:GetHandler():GetSequence());
end
...
function this.costfilter(c)
    return c:IsFaceup() and c:IsSetCard(0x677) and c:IsType(TYPE_MONSTER) and c:IsAbleToHandAsCost()
end
function this.thfilter(c)
    return c:IsSetCard(0x677) and c:IsType(TYPE_MONSTER) and not c:IsCode(21300205) and c:IsAbleToHand()
end
function this.thcost(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.IsExistingMatchingCard(this.costfilter,tp,LOCATION_MZONE,0,1,nil) end
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
    local tc=Duel.SelectMatchingCard(tp,this.costfilter,tp,LOCATION_MZONE,0,1,1,nil)
