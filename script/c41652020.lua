...
function s.efftg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if eff==1 then
		Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,nil,tp,LOCATION_EXTRA)
	elseif eff==2 then
...
function s.thfilter(c)
	return aux.IsCodeOrListed(c,41652000) and c:IsAbleToHand() and c:IsFaceup()
end
...
function s.effop2(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif eff==1 then
		...
		if ct>0 then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
			local tg=Duel.SelectMatchingCard(tp,s.thfilter,tp,LOCATION_REMOVED,0,1,2,nil)
			if #tg>0 then
				Duel.SendtoHand(tg,nil,REASON_EFFECT)
				Duel.ConfirmCards(1-tp,tg)
			end
