...
function s.tg1(e,tp,eg,ep,ev,re,r,rp,chk)
    ...
    if chk==0 then
        if qp then return Duel.IsExistingMatchingCard(s.filter1,tp,LOCATION_DECK,0,1,nil)
            or Duel.IsExistingMatchingCard(Card.IsAbleToHand,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
...
function s.op1(e,tp,eg,ep,ev,re,r,rp)
    ...
    local b2=e:GetLabel()==1 and Duel.IsExistingMatchingCard(Card.IsAbleToHand,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil)
    ...
    elseif b2 then
        ...
        if #g>0 then Duel.HintSelection(g) Duel.SendtoHand(g,nil,REASON_EFFECT) end
    end
end
