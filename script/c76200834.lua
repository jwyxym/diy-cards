...
function s.initial_effect(c)
	aux.AddCodeList(c,76200832,76200848)
	--select effect
...
function s.filter(c,e,tp)
	return c:IsSetCard(0x728)
end
function s.mfilter(c)
	return c:GetLevel()>0 and (c:IsSetCard(0x728) or c:IsType(TYPE_FLIP)) and c:IsReleasable(REASON_EFFECT|REASON_MATERIAL|REASON_RITUAL)
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local mg1=Duel.GetRitualMaterial(tp):Filter(s.mfilter,nil)
	local mg2=Duel.GetMatchingGroup(s.mfilter,tp,LOCATION_DECK,0,nil)
	local s1=Duel.IsExistingMatchingCard(aux.RitualUltimateFilter,tp,LOCATION_DECK,0,1,nil,s.filter,e,tp,mg1,nil,Card.GetLevel,"Greater")
...
function s.spop1(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.RitualUltimateFilter,tp,LOCATION_DECK,0,1,1,nil,s.filter,e,tp,mg1,nil,Card.GetLevel,"Greater")
	local tc=g:GetFirst()
	if tc then
...
function s.cfilter(c)
	return c:IsFaceup() and c:IsCode(76200832,76200848)
end
