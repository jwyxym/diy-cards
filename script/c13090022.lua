...
function c13090022.initial_effect(c)
	...
	e1:SetCountLimit(1,13090022+EFFECT_COUNT_CODE_OATH)
...
local e4=Effect.CreateEffect(c)
	...
	e4:SetCountLimit(1,13091022+EFFECT_COUNT_CODE_OATH)
	e4:SetTarget(cm.tg)
...
if chk==0 then return Duel.IsExistingMatchingCard(function(c) return not c:IsSetCard(0xe08) and not c:IsPublic() end,tp,LOCATION_HAND,0,1,nil)
end
end
function cm.con(e,tp,eg,ep,ev,re,r,rp,chk)
return Duel.IsExistingMatchingCard(cm.penfilter,tp,LOCATION_ONFIELD,0,1,e:GetHandler())
end
...
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=g:GetFirst()
	if tc:IsType(TYPE_PENDULUM) and Duel.SelectYesNo(tp,aux.Stringid(13090004,0)) then
