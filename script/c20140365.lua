...
function s.filter1a(c)
    return c:IsSetCard(0x2b1) and c:IsAbleToGrave()
end
function s.filter1b(c)
    return c:IsSetCard(0x2b1) and c:IsAbleToRemove()
end
function s.tg1(e,tp,eg,ep,ev,re,r,rp,chk)
    ...
    if chk==0 then
        if qp then return (Duel.IsExistingMatchingCard(s.filter1a,tp,LOCATION_DECK,0,1,nil)
                or Duel.IsExistingMatchingCard(s.filter1b,tp,LOCATION_DECK,0,1,nil))
            or Duel.IsExistingMatchingCard(Card.IsAbleToDeck,tp,LOCATION_GRAVE+LOCATION_REMOVED,LOCATION_GRAVE+LOCATION_REMOVED,1,nil) end
...
function s.op1(e,tp,eg,ep,ev,re,r,rp)
    local b1=Duel.IsExistingMatchingCard(s.filter1a,tp,LOCATION_DECK,0,1,nil)
        or Duel.IsExistingMatchingCard(s.filter1b,tp,LOCATION_DECK,0,1,nil)
    ...
    if b1 and (not b2 or not Duel.SelectYesNo(tp,aux.Stringid(id,6))) then
        ...
        if #g>0 then
            local tc=g:GetFirst()
            if tc:IsAbleToRemove() and (not tc:IsAbleToGrave() or Duel.SelectOption(tp,aux.Stringid(id,2),aux.Stringid(id,3))==1) then
    ...
    elseif b2 then
        ...
        if #sg>0 then Duel.HintSelection(sg) Duel.SendtoDeck(sg,nil,SEQ_DECKSHUFFLE,REASON_EFFECT) end
    end
end
