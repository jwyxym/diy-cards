...
function cm.op(e,tp,eg,ep,ev,re,r,rp)
	...
	if mg:GetCount()>0 then
		Duel.BreakEffect()
		local tg1=Duel.GetMatchingGroup(aux.RitualUltimateFilter,tp,LOCATION_HAND,0,nil,nil,e,tp,mg,nil,Card.GetLevel,"Greater")
		if not tg1 then tg1=Group.CreateGroup() end
		local tg2=g:Filter(aux.RitualUltimateFilter,nil,nil,e,tp,mg,nil,Card.GetLevel,"Greater")
		if tg2 then tg1:Merge(tg2) end
		if tg1:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(m,2)) then
