...
function s.hspfilter(c,tp,sc)
	return c:IsCode(9028399) and c:IsAbleToRemoveAsCost(POS_FACEDOWN)
...
function s.thfilter(c)
	return c:IsFaceupEx() and c:IsSetCard(0x597) and c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsSSetable()
end
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>1 then
		...
		if #g>0 and Duel.GetLocationCount(tp,LOCATION_SZONE)>0 and Duel.SelectYesNo(tp,aux.Stringid(id,0)) then
			...
			local ft=math.min(2,Duel.GetLocationCount(tp,LOCATION_SZONE),#g)
...
function s.spfilter(c,e,tp)
	return c:IsFaceupEx() and not c:IsAttribute(ATTRIBUTE_EARTH) and c:IsSetCard(0x597)
