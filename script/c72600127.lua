...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if sg1:GetCount()>0 or (sg3~=nil and sg3:GetCount()>0) then
		...
		if sg1:IsContains(tc) and (sg3==nil or not sg3:IsContains(tc) or not Duel.SelectYesNo(tp,ce:GetDescription())) then
			if tc:IsRace(RACE_ILLUSION) then
...
function s.spsfilter(c,e,tp)
	return c:IsCode(72600121) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.spstg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and s.spsfilter(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(s.spsfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,s.spsfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
