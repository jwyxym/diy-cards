...
function s.rfilter(c,tp)
	return ((c:GetOriginalAttribute()&ATTRIBUTE_DARK>0 and not c:IsLocation(LOCATION_MZONE)) or c:IsAttribute(ATTRIBUTE_DARK)) and (c:IsControler(tp) or c:IsFaceup()) and c:IsAbleToRemoveAsCost()
end
...
function s.matfilter(c)
	return c:IsCanOverlay() or c:IsAbleToRemove()
end
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	local req=ct*3
	if req>0 and Duel.IsExistingMatchingCard(s.matfilter,tp,0,LOCATION_HAND+LOCATION_ONFIELD+LOCATION_EXTRA,req,nil) then
		if Duel.IsExistingMatchingCard(s.matfilter2,tp,0,LOCATION_HAND+LOCATION_ONFIELD+LOCATION_EXTRA,req,nil) and Duel.SelectYesNo(1-tp,aux.Stringid(id,2)) then
			Duel.Hint(HINT_SELECTMSG,1-tp,HINTMSG_XMATERIAL)
			local g=Duel.SelectMatchingCard(1-tp,s.matfilter2,tp,0,LOCATION_HAND+LOCATION_ONFIELD+LOCATION_EXTRA,req,req,nil)
			Duel.Overlay(c,g)
		else
			Duel.Hint(HINT_SELECTMSG,1-tp,HINTMSG_REMOVE)
			local g=Duel.SelectMatchingCard(1-tp,s.matfilter,tp,0,LOCATION_HAND+LOCATION_ONFIELD+LOCATION_EXTRA,req,req,nil)
