...
function s.rfilter(c,tp)
	return ((c:GetOriginalAttribute()&ATTRIBUTE_FIRE>0 and not c:IsLocation(LOCATION_MZONE)) or c:IsAttribute(ATTRIBUTE_FIRE)) and (c:IsControler(tp) or c:IsFaceup()) and c:IsAbleToRemoveAsCost()
end
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	local ct=math.floor(e:GetHandler():GetAttack()/2000)
	if ct>0 and Duel.IsExistingMatchingCard(nil,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,ct,nil) then
