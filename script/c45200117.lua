...
local seq = {LINK_MARKER_BOTTOM, LINK_MARKER_BOTTOM_LEFT, LINK_MARKER_LEFT, LINK_MARKER_BOTTOM_LEFT, LINK_MARKER_BOTTOM, LINK_MARKER_BOTTOM_RIGHT, LINK_MARKER_RIGHT}
...
function s.attfilter(c)
	return c:IsAttribute(ATTRIBUTE_WIND+ATTRIBUTE_WATER+ATTRIBUTE_DARK)
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Duel.IsExistingMatchingCard(s.linkfilter,tp,LOCATION_EXTRA,0,1,nil)
			...
			and Duel.IsExistingMatchingCard(Card.IsType,tp,0,LOCATION_SZONE,1,nil,TYPE_SPELL+TYPE_TRAP)
	end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local dg=Duel.GetMatchingGroup(Card.IsType,tp,0,LOCATION_SZONE,nil,TYPE_SPELL+TYPE_TRAP)
	if #dg>0 then
