...
function cm.spc(c,e,tp)
	return c:IsSetCard(0xccf) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function cm.thc(c)
	return c:IsSetCard(0xccf) and c:IsAbleToHand() and c:IsType(TYPE_SPELL+TYPE_TRAP)
end
function cm.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tg:FilterCount(function (c)
						return c:IsSetCard(0xccf) and c:IsType(TYPE_MONSTER) end,nil)>=2 then
		Duel.ShuffleDeck(tp)
		Duel.BreakEffect()
