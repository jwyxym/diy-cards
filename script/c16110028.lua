...
function cm.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=Duel.GetMatchingGroup(cm.ctlfilter,tp,0,LOCATION_MZONE,nil)
	if chkc then return cm.filter(chkc,ct1,ct2) and chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) end
	if chk==0 then return Duel.IsExistingTarget(cm.filter2,tp,0,LOCATION_MZONE,1,nil,ct1,ct2) end
