...
function s.matfilter(c)
	return c:IsRace(RACE_SEASERPENT) or c:IsRace(RACE_WYRM)
end
...
function s.efffilter(c)
	return c:IsSetCard(0x1e92) and c:GetOriginalType()&TYPE_MONSTER>0
end
function s.effop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=g:GetFirst()
	while tc do
		tc:RegisterFlagEffect(id,nil,EFFECT_FLAG_CLIENT_HINT,1,0,aux.Stringid(id,1))
		tc=g:GetNext()
	end
end
function s.cfilter(c,e,tp)
	return c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousControler(1-e:GetHandlerPlayer())
end
...
function s.eqfilter(c)
	return c:IsSetCard(0xe92,0xd91) and c:IsType(TYPE_MONSTER) and not c:IsForbidden()
end
function s.eqtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsMainPhase() and Duel.GetLocationCount(tp,LOCATION_SZONE)>0 and Duel.IsExistingMatchingCard(s.eqfilter,tp,LOCATION_DECK+LOCATION_EXTRA,0,1,nil) and Duel.IsExistingMatchingCard(Card.IsFaceup,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
end
function s.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	local ec2=Duel.SelectMatchingCard(tp,s.eqfilter,tp,LOCATION_DECK+LOCATION_EXTRA,0,1,1,nil):GetFirst()
	if ec1 and ec2 then
