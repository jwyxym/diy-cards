...
function s.initial_effect(c)
	c:EnableReviveLimit()
    aux.AddXyzProcedureLevelFree(c,s.xyzfilter,nil,2,99)
    --when atk
...
function s.xtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
    ...
    if chk==0 then return #mat>0 and Duel.IsExistingTarget(s.matfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,c)
        and Duel.IsExistingMatchingCard(s.tdfilter,tp,LOCATION_SZONE,0,1,nil) end
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_XMATERIAL)
    if #mat<=2 then
        Duel.SelectTarget(tp,nil,tp,LOCATION_MZONE,LOCATION_MZONE,1,#mat,nil)
    else
...
function s.mttg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
    ...
    if chk==0 then return c:IsType(TYPE_XYZ) and Duel.IsExistingTarget(s.filter,tp,LOCATION_ONFIELD,0,1,c) end
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_XMATERIAL)
    local g=Duel.SelectTarget(tp,s.filter,tp,LOCATION_ONFIELD,0,1,2,c)
    g:KeepAlive()
