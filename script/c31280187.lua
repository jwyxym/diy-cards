...
function s.discon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsChainNegatable(ev) and (re:IsActiveType(TYPE_MONSTER) or re:IsHasType(EFFECT_TYPE_ACTIVATE))
    	and Duel.IsExistingMatchingCard(s.confilter,tp,LOCATION_ONFIELD,0,1,nil) and rp~=tp
end
function s.disop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.NegateActivation(ev) and rc:IsRelateToEffect(re)
        ...
        local g=Duel.GetMatchingGroup(s.confilter,tp,LOCATION_ONFIELD+LOCATION_EXTRA,0,nil)
