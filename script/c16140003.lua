...
function cm.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	aux.AddFusionProcCode3(c,16140000,16140001,16140002,true,true)
...
function cm.atktg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		return not c:IsAttack(atk) and c:GetFlagEffect(m)==0
	end
...
function cm.spfilter(c,e,tp)
	return rk.check(c,"TRUENAME") and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function cm.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return not Duel.IsPlayerAffectedByEffect(tp,59822133)
		and Duel.GetLocationCount(tp,LOCATION_MZONE)>=3
		and Duel.IsExistingMatchingCard(cm.spfilter,tp,0x13,0,3,nil,e,tp) end
