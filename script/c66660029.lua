...
function cm.filter(c,e,tp)
	return c:IsSetCard(0x666) and c:IsType(TYPE_SYNCHRO) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and ((Duel.GetLocationCountFromEx(tp,tp,nil,c)>0 and c:IsFaceup()) or (Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and c:IsFacedown()))
end
