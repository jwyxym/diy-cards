...
function s.filter1(c,e,tp)
	local rk=c:GetRank()
	return rk>0 and c:IsFaceup() and c:IsRace(RACE_FIEND+RACE_ZOMBIE)
...
function s.filter2(c,e,tp,mc,rk)
	...
	return c:IsRank(rk) and c:IsAttribute(ATTRIBUTE_DARK)
		and c:IsRace(RACE_FIEND+RACE_ZOMBIE) and mc:IsCanBeXyzMaterial(c)
