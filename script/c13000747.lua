...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local bb=g:GetNext():GetAttribute()
	if g:GetCount()>0 and Duel.SendtoHand(g,nil,REASON_EFFECT)~=0 then
		Duel.ConfirmCards(1-tp,g)
		if tg:IsExists(s.filter9,1,nil,tp,aa,bb) then
		   if Duel.SelectYesNo(tp,aux.Stringid(13000747,0)) then
		   local ta=tg:SelectSubGroup(tp,s.filter9,true,1,1,tp,aa,bb)
		   Duel.Destroy(ta,REASON_EFFECT)
	end
end
end
end
function s.filter9(c,tp,aa,bb)
	return Card.IsPublic(c) and (c:IsAttribute(aa) or c:IsAttribute(bb))
end
