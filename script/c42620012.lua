...
function cm.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetFieldGroupCount(tp,0x04,0)==0 and Duel.GetFieldGroupCount(tp,0,0x04)>0
end
...
function cm.sptg1(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if re:GetHandler():IsAbleToRemove(tp,POS_FACEDOWN) and re:GetHandler():IsRelateToEffect(re) then
		eg:AddCard(c)
...
function cm.spop1(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.Remove(c,POS_FACEUP,REASON_EFFECT) and Duel.NegateEffect(ev) and re:GetHandler():IsRelateToEffect(re) then
