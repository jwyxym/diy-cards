...
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.cfilter,tp,LOCATION_HAND+LOCATION_ONFIELD,0,2,e:GetHandler(),tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectMatchingCard(tp,s.cfilter,tp,LOCATION_HAND+LOCATION_ONFIELD,0,2,2,e:GetHandler(),tp)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 then
		...
		if Duel.IsPlayerCanDraw(tp,1) and e:GetLabel()==1
