...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		return Duel.IsExistingMatchingCard(aux.RitualUltimateFilter,tp,LOCATION_DECK,0,1,nil,s.ritfilter,e,tp,mg1,mg2,Card.GetLevel,"Equal")
	end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.RitualUltimateFilter,tp,LOCATION_DECK,0,1,1,nil,s.ritfilter,e,tp,mg1,mg2,Card.GetLevel,"Equal")
	local tc=g:GetFirst()
	if tc then
		local mg=mg1
		mg:Merge(mg2)
		...
		if tc:IsCode(74400916) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
			Duel.BreakEffect()
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
			local sg=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.spfilter),tp,LOCATION_DECK,0,1,1,nil,e,tp)
			if #sg>0 then
...
function s.spfilter(c,e,tp)
	return aux.IsSetNameMonsterListed(c,0x742) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP)
end
