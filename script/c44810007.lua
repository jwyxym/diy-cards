...
function s.daop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and Duel.Destroy(c,REASON_EFFECT)>0 then
		...
		if not Duel.SelectYesNo(tp,aux.Stringid(id,5)) then
		...
		if g:IsExists(s.rmfilter,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(id,3)) then
		...
		if #sg>0 then
			...
			if Duel.SendtoHand(sg,tp,REASON_EFFECT)~=0 then
				...
				if g:IsExists(s.rmfilter1,1,sg) and Duel.SelectYesNo(tp,aux.Stringid(id,4)) then
				Duel.BreakEffect()
				Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
				Duel.RevealSelectDeckSequence(true)
				local sg1=g:FilterSelect(tp,s.rmfilter1,1,2,sg)
				Duel.RevealSelectDeckSequence(false)
				if #sg1>0 then
					Duel.DisableShuffleCheck(true)
