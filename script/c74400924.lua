...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		return Duel.IsExistingMatchingCard(aux.RitualUltimateFilter,tp,LOCATION_GRAVE,0,1,nil,s.ritfilter,e,tp,mg1,mg2,Card.GetLevel,"Equal")
	end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.RitualUltimateFilter,tp,LOCATION_GRAVE,0,1,1,nil,s.ritfilter,e,tp,mg1,mg2,Card.GetLevel,"Equal")
	local tc=g:GetFirst()
	if tc then
		local mg=mg1
		mg:Merge(mg2)
