...
function s.spfilter(c,e,tp)
	return c:IsSetCard(0x516) and c:IsLevel(2) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.cfilter(c,se)
	return c:IsReason(REASON_BATTLE+REASON_EFFECT) and c:IsType(TYPE_FUSION)
		and bit.band(c:GetPreviousRaceOnField(),RACE_SPELLCASTER)~=0 and c:IsPreviousPosition(POS_FACEUP)
...
function s.thfilter(c)
	return c:IsCode(51900630) and c:IsAbleToHand()
end
