...
function s.linkcon(e,c,og,lmat,min,max)
    ...
    if not s.has_setcode(g, 0x909) then return false end
    return s.total_link(g) >= 4
end
function s.linkop(e,tp,eg,ep,ev,re,r,rp,c,og,lmat,min,max)
    if c==nil then return end
    local g = Duel.GetMatchingGroup(Card.IsFaceup, tp, LOCATION_MZONE, 0, nil)
    if not s.has_setcode(g, 0x909) then return end

    Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_XMATERIAL)
    local sg = g:Select(tp, 2, #g, nil)
    if #sg < 2 then return end
    if not s.has_setcode(sg, 0x909) then return end
...
function s.spop2(e,tp,eg,ep,ev,re,r,rp)
    ...
    if #g1 > 0 then
        ...
        local sg1 = g1:Filter(Card.IsSetCard, nil, 0x908):Select(tp, 0, 1, nil)
        if #sg1 > 0 then
    ...
    if #g2 > 0 then
        Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
        local sg2 = g2:Filter(Card.IsSetCard, nil, 0x907):Select(tp, 0, 1, nil)
        if #sg2 > 0 then
