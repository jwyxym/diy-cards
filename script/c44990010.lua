...
function s.gfilter(c)
	return c:IsSetCard(0xcf0) and c:IsType(TYPE_MONSTER)
end
function s.regop(e,tp,eg,ep,ev,re,r,rp)
	for tc in aux.Next(eg) do
		...
		if s.regfilter(tc) and Duel.GetFlagEffect(tp,id+code)==0 then
			...
			e1:SetDescription(aux.Stringid(id,code))
...
function s.wintg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local g=Duel.GetMatchingGroup(s.regfilter,tp,LOCATION_GRAVE+LOCATION_ONFIELD+LOCATION_REMOVED,0,nil)
...
function s.winop(e,tp,eg,ep,ev,re,r,rp)
	local mg=Duel.GetMatchingGroup(s.gfilter,tp,LOCATION_GRAVE+LOCATION_ONFIELD+LOCATION_REMOVED,0,nil)
	if mg:GetCount()<9 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local sg=mg:SelectSubGroup(tp,aux.dncheck,true,9,9)
		if sg and sg:GetCount()==9 and Duel.SendtoDeck(sg,nil,SEQ_DECKSHUFFLE,REASON_SPSUMMON)==9 then
