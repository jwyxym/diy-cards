...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	local e2=aux.AddRitualProcGreater2(c,s.ssfilter,LOCATION_HAND+LOCATION_GRAVE,nil,s.mfilter,true)
	e2:SetDescription(aux.Stringid(id,2))
	e2:SetCountLimit(1,id+o)
	c:RegisterEffect(e2)
end
function s.filter1(c,e)
	return (c:IsLocation(LOCATION_REMOVED+LOCATION_ONFIELD+LOCATION_GRAVE) or c:IsFaceup()) and c:IsType(TYPE_MONSTER)
		and c:IsCanBeFusionMaterial() and c:IsAbleToDeck() and not c:IsImmuneToEffect(e)
end
function s.filter2(c,e,tp,m,f,chkf)
	return c:IsType(TYPE_FUSION) and c:IsAttribute(ATTRIBUTE_LIGHT+ATTRIBUTE_EARTH) and (not f or f(c))
		and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,false,false) and c:CheckFusionMaterial(m,nil,chkf)
end
function s.fcheck(tp,sg,fc)
	return sg:IsExists(Card.IsType,1,nil,TYPE_FLIP)
end
...
function s.cfilter(c)
	return c:IsLocation(LOCATION_GRAVE+LOCATION_HAND) or (c:IsLocation(LOCATION_MZONE) and c:IsFaceup())
end
function s.ssfilter(c)
	return c:IsSetCard(0x728) and c:IsType(TYPE_MONSTER)
end
function s.mfilter(c)
	return (c:IsSetCard(0x728) and c:IsType(TYPE_MONSTER)) or c:IsType(TYPE_FLIP)
end
